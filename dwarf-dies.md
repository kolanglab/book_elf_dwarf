...
そこで DWARF は、この「DIE の雛形」を**略号** (abbreviation) として `.debug_abbrev` に一度だけ定義し、`.debug_info` の各 DIE では**略号の番号と、実際の値だけ**を書きます [](#cite:eager2012)。プログラミングでいえば、構造体の「定義」を 1 か所に書き、あとは「この型のインスタンス、値はこれ」と並べるようなものです。
