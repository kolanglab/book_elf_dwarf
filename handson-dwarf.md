...
ここまでは、ディスク上の ELF ファイルを `fopen` で開いて読みました。最後のハンズオンでは、読む相手を**いま動いているプロセス自身**に変えます。`.symtab` と `.debug_line` を自分のプロセスから読み、スタックを巻き戻して、`backtrace()` よりずっとリッチな ―― 関数名と行番号つきの、しかも全スレッド分の ―― バックトレースを自作します。
