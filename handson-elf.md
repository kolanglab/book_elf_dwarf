...
ELF を読むツールが書けました。ここで書いた「セクションを名前で探して中身を読む」処理は、次のハンズオンで `read_section` という関数にまとめ直し、`.debug_line` の取り出しにそのまま再利用します。次章では、いま 0 番地扱いだった `.debug_line` セクションの中身に踏み込み、アドレスからソース行を求める処理を実装します。いよいよ DWARF を「実行」します。
