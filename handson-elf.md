...
ELF を読むツールが書けました。ここで得た「セクションを名前で探して中身を読む」部品 ―― とくに `read_section` のような関数 ―― は、次のハンズオンでそのまま再利用します。次章では、いま 0 番地扱いだった `.debug_line` セクションの中身に踏み込み、アドレスからソース行を求める処理を実装します。いよいよ DWARF を「実行」します。
