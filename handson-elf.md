...
`memcmp(eh.e_ident, ELFMAG, SELFMAG)` が、第 3 章のマジックナンバー検査です。`ELFMAG` は `"\x7fELF"`、`SELFMAG` はその長さ 4 です。先頭 4 バイトが一致しなければ ELF ではないと判断します。続いて `eh.e_ident[EI_CLASS]` を見て、64 ビット (`ELFCLASS64`) でなければ、ここでは扱わないことにします。これらは第 3 章の `e_ident` の説明そのものです。なお、厳密には `e_ident[EI_DATA]` のエンディアン確認も加えるべきところです。本書のハンズオンは一貫して「x86-64・リトルエンディアンのホストで、同じ環境向けの ELF を読む」前提なので省いていますが、ビッグエンディアンの ELF も読める汎用パーサを目指すなら、`EI_DATA` を見て多バイト値の読み方を切り替える必要があります。
...
ELF を読むツールが書けました。ここで書いた「ELF ヘッダとプログラム／セクションヘッダを配列として読む」骨格は、続くハンズオンでそのまま土台になります。次章ではまず、いま読み取った**プログラムヘッダ**を使って、ELF を実際にメモリへ展開し実行します。これは OS のローダがやっていることを自分の手で再現する作業です。いよいよ ELF を「動かし」ます。
